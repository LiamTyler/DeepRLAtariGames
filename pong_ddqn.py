...
class Network:
    ...
    def buildModel(self):
        model = Sequential()
        model.add(Conv2D(32, (8, 8), strides=(4,4), activation='relu', input_shape=(self.inputSize), data_format='channels_first'))
...
class Agent:
    ...
    def experienceReplay(self):
        ...
        s = self.currentNetwork.inputSize
...
class Environment:
    ...
    def run(self, agent, numEpisodes, verbose = True, render = False):
        for episode in range(numEpisodes):
            ...
            stateStack = np.array([state, state])
            score = 0            
            done = False
            ...
            while not done:
                ...
                newStateStack = np.array([stateStack[1], newState])
                
                # store experience in replay memory
                agent.observe(stateStack, action, reward, newStateStack, done)
...
class Pong(Environment):
    def __init__(self):
        ...
        self.stateSpaceShape = (2, 80, 80)

    def processState(self, I):
      """ prepro 210x160x3 uint8 frame into 6400 (80x80) 1D float vector """
      I = I[35:195] # crop to 160x160x3
      I = I[::2,::2,0] # downsample by factor of 2 to 80x80x1
...
env = Pong()
replayBuffer = PERReplayBuffer(50000)
randomAgent = RandomAgent(replayBuffer)
#while not replayBuffer.isFull():
while replayBuffer.size() < 1000:
    env.run(randomAgent, 1, False, False)
    print(replayBuffer.size() / 50000)
print("full")

tNetwork = Network(0.00025, env.stateSpaceShape, env.actionSpaceSize)
